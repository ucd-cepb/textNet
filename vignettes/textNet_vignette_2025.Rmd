...
## Installation
The stable version of this package can be installed from Github, using the *pak* package [@csardi_pak_2024]:
...
```{r, out.width = "400px"}
knitr::include_graphics("vignette_figures/figure-markdown_strict/flowchart.png")
```
...
library(textNet)
# example_plan_v1_parsed and example_plan_v2_parsed are available automatically
# after library(textNet) via LazyData

```

```{r extract, results = 'hide'}
#extract entity networks from each plan version
...
#or a target node
extracts <- lapply(
  list(example_plan_v1_parsed, example_plan_v2_parsed),
  function(parsed) {
    textnet_extract(parsed,
...
old_acronyms <- find_acronyms(example_plan_v1_text)
new_acronyms <- find_acronyms(example_plan_v2_text)
...
```{r viz_note, echo=FALSE, results='asis', eval=!has_viz}
cat("**Note:** The plots in this vignette require the ggraph, ggplot2, and gridExtra packages. Install them to see the visualizations: `install.packages(c('ggraph', 'ggplot2', 'gridExtra'))`")
```

```{r plot, message=F, warning=F, fig.width=5,fig.height=5, fig.retina=2, eval=has_viz}
library(ggraph)
...
```{r composite_plot, fig.width=5,fig.height=5, fig.retina=2, eval=has_viz}
set.seed(50000)
...
The 2x2 table below summarizes the rate at which each entity type is found in both plans.
...
```{r step8b4, warning=F, message=F, fig.width=10,fig.height=8, fig.retina=2, eval=has_viz}
library(gridExtra)
library(ggplot2)
...
```{r pdf_clean, eval = FALSE}
library(textNet)
library(stringr)
#download sample pdf data from state website
#this is the same as inst/extdata/example_plan_v1.pdf and inst/extdata/example_plan_v2.pdf
...
#download.file(URL, destfile = "inst/extdata/example_plan_v1.pdf", method="curl")

#URL <- "https://sgma.water.ca.gov/portal/service/gspdocument/download/9625"
#download.file(URL, destfile = "inst/extdata/example_plan_v2.pdf", method="curl")

pdfs <- c(system.file("extdata", "example_plan_v1.pdf", package = "textNet"),
            system.file("extdata", "example_plan_v2.pdf", package = "textNet"))

#this is the equivalent of the textNet::example_plan_v1_text and
#textNet::example_plan_v2_text sample data that travels with the package
plan_texts <- textNet::pdf_clean(pdfs, ocr=F, maxchar=10000,
                     export_paths=NULL, return_to_memory=T, suppressWarn = F,
                     auto_headfoot_remove = T)
example_plan_v1_text <- plan_texts[[1]]
example_plan_v2_text <- plan_texts[[2]]
```

The above code produces results equivalent to the bundled sample data, which can be loaded directly:

```{r pdf_clean_load, echo = TRUE}
library(textNet)
library(stringr)
```
...
#this is the equivalent of the textNet::example_plan_v1_parsed and
#textNet::example_plan_v2_parsed sample data that travels with the package
#it is a thin wrapper for spacyr::spacy_parse
plan_parsed <- textNet::parse_text(
                          keep_hyph_together = F,
                          phrases_to_concatenate = water_bodies,
                          concatenator = "_",
                          text_list = list(example_plan_v1_text, example_plan_v2_text),
                                 parsed_filenames=c("old_parsed","new_parsed"),
                                 overwrite = T,
                          custom_entities = list(WATER = water_bodies))
example_plan_v1_parsed <- plan_parsed[[1]]
example_plan_v2_parsed <- plan_parsed[[2]]

```

The above code produces results equivalent to the bundled sample data objects
(`example_plan_v1_parsed`, `example_plan_v2_parsed`), which are available
automatically after `library(textNet)` via LazyData.
