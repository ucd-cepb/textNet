...
<!-- NOTE: This script requires spacyr, reticulate, and a working spaCy
     Python environment. It is not built as a vignette because these are
     suggested (not required) dependencies. Run it manually on a machine
     with spacyr configured. -->
...
#that are included in the package, EXCEPT the package versions have pages that are maps 
#or figures removed to shrink file size.
...
```{r plot, message=F, warning=F, fig.width=5,fig.height=5, fig.retina=2}
library(ggraph)
set.seed(50000)
...
```{r step8b4, eval=FALSE, fig.width=8,fig.height=6, fig.retina=2}
library(gridExtra)
library(ggplot2)
